...
library(pmcalibration) # version 0.1.0 (Download from CRAN)
# https://github.com/stephenrho/pminternal
library(pminternal) # version 0.0.1
library(ClinicalUtilityRecal) # version 0.1.0 (Download from CRAN)
library(rms) # version 6.8-1 (Download from CRAN)
...
```{r echo=TRUE, eval = FALSE}
# Note: Setting a seed guarantees perfect reproducibility of the computation procedure. This is especially important, when random processes (based on the computer) are involved.
set.seed(1)
...
unique(unlist(lapply(TrainLs, FUN=function(x) mean(x$newAud))))
unique(unlist(lapply(TestLs, FUN=function(x) mean(x$newAud))))
```
...
endTime <- Sys.time()
difftime(endTime, startTime) # Approx. 40 seconds.
```
...
It is recommended (e.g., Vickers et al., 2019) to select a reasonable range of threshold probabilities for the envisioned treatment, due to the fact that people differ regarding their estimation of health risks, i.e., whether to take action or whether to wait.
...
# Output in R console (based on the original data)
  thrsh  Median    Mean      Min     qu1     qu3    Max
  <dbl>   <dbl>   <dbl>    <dbl>   <dbl>   <dbl>  <dbl>
1  0.01 0.0225  0.0223  0.0195   0.0216  0.0230  0.0239
2  0.02 0.0171  0.0171  0.0104   0.0156  0.0189  0.0218
3  0.03 0.0123  0.0122  0.00509  0.0103  0.0139  0.0196
4  0.04 0.00866 0.00859 0.000684 0.00650 0.0101  0.0163
5  0.05 0.00663 0.00677 0.000648 0.00456 0.00845 0.0140

allDCALs$rfNB
# Output in R console (based on the original data)
  thrsh  Median    Mean      Min     qu1     qu3    Max
  <dbl>   <dbl>   <dbl>    <dbl>   <dbl>   <dbl>  <dbl>
1  0.01 0.0224  0.0225   0.0196  0.0218  0.0231  0.0255
2  0.02 0.0165  0.0166   0.0115  0.0150  0.0182  0.0220
3  0.03 0.0119  0.0119   0.00540 0.0102  0.0138  0.0180
4  0.04 0.00812 0.00824  0.00114 0.00650 0.0103  0.0151
5  0.05 0.00504 0.00503 -0.00144 0.00293 0.00671 0.0141
```
...
Produce the same ggplot for random forest, if you want to. Additionally, if wanted, compute some calibration summary results. They are way less relevant, compared to clinical utility results, because they treat the whole breadth of predicted probabilities as if they are all equally important. However, this is most likely never the case in clinical reality, which is the reason why a 'reasonable range of threshold probabilities' must be selected as part of a decision curve analysis (see DCA description above).
...
**Note**. All of the following results refer solely to the logistic regression model, because we did not use the random forest model as a competitor to logistic regression. Detailed reasons can be found in the publication and the other supplementary materials.

There exists an alternative to k-fold cross-validation, which makes use of the full data. This bootstrapping alternative is described in Box 1 of Riley and Collins (2023), and in Box 2 of Collins et al. (2024). The procedure is implemented in the R package pminternal (Rhodes, Version 0.0.1). The main advantage of this approach is that the user receives an estimate of the overoptimism of the apparent prediction performance ('apparent' performance are the results, after using the same dataset for training and for testing the prediction model).

```{r echo=TRUE, eval = FALSE}
# Take the total dataset, then fit a logistic regression model to it.
mod <- glm(newAud ~ ., family = binomial(link="logit"), data = d)
...
Apparent  0.78  0.02887      0.00 1.000 0.0064 0.0042 0.0138 0.077 0.0112
Optimism  0.02 -0.00062      0.17 0.061 0.0018 0.0011 0.0059 0.049 0.0093
Corrected 0.76  0.02949     -0.17 0.939 0.0046 0.0031 0.0079 0.028 0.0019
```

The results show C, which is a measure of discrimination, whereas the remaining measures all belong the the class of calibration measures. Since our main interest is the net benefit, we modified the function 'boot_optimism' of the package pminternal. The modified function 'boot_optimismDCA' is part of this predictAUDPsyCoLaus R package.
...
set.seed(1)
dcaCorrected <- boot_optimismDCA(data=d, outcome = "newAud", B=500, thresholds = seq(.01, .05, by=.01))
...
binary_vars <- c("newAud", "ADAPU2", "inactivity", "MDDPD2", "Sex", "MARIE")
# Number of repetitions of the mimicking simulations.
modgoReps <- 500
# Run the mimicking simulations.
...
```{r echo=TRUE, eval = FALSE}
# d <- readRDS(file="PsyCoLaus3654.rds") # Original data
# pwts: prediction weights
pwts <- coefficients(glm(newAud ~ ., family = binomial(link="logit"), data=d))
idxWithOutcome <- which(dSim$newAud == 1)
idxWithoutOutcome <- which(dSim$newAud == 0)
set.seed(9)
...
for(i in seeds) {
    ...
    mod_i <- glm(newAud ~ ., family = binomial(link="logit"), data=d_i)
    # Prediction weights
...
# saveRDS(object=d_i, file = "PsyCoLausMimic4438.rds")
# d4438 <- readRDS(file="PsyCoLausMimic4438.rds")
```
...
mod4438 <- glm(newAud ~ ., family = binomial(link="logit"), data = d4438)
# Next, run the function validate from the pminternal R package.
start <- Sys.time()
set.seed(1)
# Set the recommended 500 bootstrap repetitions.
...
The respective functions within the pminternal R package have been modified by us, because we wanted to visualize the results with ggplot2, instead of the R graphics package. All code examples of model instability use the variable 'val1'.
...
p <- 
    ...
    ylab(label="Estimated risk from bootstrap models (n = 500)") +
    theme(
        panel.background = element_blank(),
        axis.text.x=element_text(size=16),
        axis.title.x=element_text(size=16),
...
p <- 
    ...
    geom_point(col="grey") +
    xlab(label="Estimated risk from development model") +
    ylab(label="Mean absolute prediction error (MAPE)") +
    theme(
        panel.background = element_blank(),
        axis.text.x=element_text(size=16),
        axis.title.x=element_text(size=16),
...
curveOrigLs <- mysml:::dca(inputDataset = data.frame(y=d$newAud, p=mod$fitted.values),
...
p1 <- p +
    geom_line(data=curvesDf, aes(x=threshold, y=net_benefit, colour=Source, linetype=Source)) +
    labs(colour=NULL) +
    labs(linetype=NULL) +
    scale_colour_manual(values = useColor) +
    scale_linetype_manual(values = useLinetype) +
    ...
    xlab(label="Threshold probabilities") +
    ylab(label="Net benefit") +
    coord_cartesian(xlim=c(0, .05), ylim=c(0,.0305)) +
    theme(
        panel.background = element_blank(),
        axis.text.x=element_text(size=16),
        axis.title.x=element_text(size=16),
...
```{r echo=TRUE, eval = FALSE}
# 5. Classification instability index (CII) (select val1 or val2)
...
p <- 
    ...
    xlab(label="Estimated risk from development model") +
    ylab(label="Classification instability index (CII)") +
    theme(
        panel.background = element_blank(),
        axis.text.x=element_text(size=16),
        axis.title.x=element_text(size=16),
...
```{r echo=TRUE, eval = FALSE}
set.seed(1)
bootDf <- boot_optimism1(data=d, outcome = "newAud", B=500)
dim(bootDf)
...
difftime(end, start) # Approx. 30 Mins.
```
...
Our prediction model for the outcome new onset of alcohol use disorder is published as part of this predictAUDPsyCoLaus R package. When you installed and loaded this predictAUDPsyCoLaus R package, run the following code, to see the detailed documentation of the prediction model:
```{r echo=TRUE, eval = FALSE}
# Load the package
...
(Intercept)    -5.40036162 0.40769544 -13.2460682 4.754680e-40
Sex            -0.20183875 0.19274980  -1.0471541 2.950285e-01
iSES15          0.12344161 0.07415749   1.6645873 9.599512e-02
MARIE          -0.29415846 0.18509473  -1.5892320 1.120080e-01
MDDPD2          0.21399425 0.18825907   1.1367009 2.556633e-01
ADAPU2         -0.11415732 0.27543898  -0.4144559 6.785402e-01
Week_ALC_type6  0.72116051 0.06247314  11.5435296 7.959096e-31
smokingstatus   0.59887445 0.11462058   5.2248422 1.743037e-07
inactivity      0.07733002 0.18799546   0.4113398 6.808234e-01
```
...
pApparent <- glm(newAud ~ ., family = binomial(link="logit"), data=d)$fitted.values
# probDistr: Probability distribution
probDistr <- data.frame(newAud=as.factor(d$newAud), p_app=pApparent)
# Display the probability distribution
p <- 
ggplot(data=probDistr, aes(x=newAud, y=p_app)) +
    geom_violin() +
    geom_jitter(height = 0, width = 0.1) +
    xlab(label="New onset alcohol use disorder (DSM-5)") +
    ylab(label="Apparent predicted probabilities") +
    theme(
        panel.background = element_blank(),
        axis.text.x=element_text(size=16),
        axis.title.x=element_text(size=16),
...
probDistrOrdered <- probDistr[order(probDistr$newAud),]
probDistrOrdered$id <- 1:nrow(probDistrOrdered)
p <- 
    ggplot(data=probDistrOrdered, aes(x=p_app, y=id, col=newAud)) +
    geom_point() +
    geom_vline(xintercept = c(.01, .05), linetype = "dashed") +
    scale_color_manual(values=c("0"="gray", "1"="red")) +
    xlab(label="Apparent predicted probabilities") +
    ylab(label="Study sample (N = 3654)") +
...
other attached packages:
 [1] predictAUDPsyCoLaus_0.1.0  modgo_1.0.1               
 [3] samplesizedev_1.0.0.0      precrec_0.14.2            
 [5] rms_6.8-1                  Hmisc_5.2-2               
 [7] ClinicalUtilityRecal_0.1.0 pminternal_0.0.1          
 [9] pmcalibration_0.1.0        gridExtra_2.3             
[11] cowplot_1.1.3              ggplot2_3.5.1             
[13] mysml_0.1.0                ranger_0.17.0             
[15] tidyr_1.3.1                magrittr_2.0.3            
[17] dplyr_1.1.4               

loaded via a namespace (and not attached):
  [1] mnormt_2.1.1         Rdpack_2.6.2         pbapply_1.7-2       
  [4] pROC_1.18.5          sandwich_3.1-0       rlang_1.1.4         
  [7] multcomp_1.4-25      polspline_1.1.25     compiler_4.4.0      
 [10] vctrs_0.6.5          reshape2_1.4.4       RcppZiggurat_0.1.6  
 [13] quantreg_5.98        stringr_1.5.1        pkgconfig_2.0.3     
 [16] fastmap_1.2.0        backports_1.5.0      rmarkdown_2.29      
 [19] prodlim_2024.06.25   nloptr_2.1.1         MatrixModels_0.5-3  
 [22] purrr_1.0.2          Rfast_2.1.0          xfun_0.50           
 [25] recipes_1.1.0        psych_2.4.6.26       parallel_4.4.0      
 [28] cluster_2.1.6        R6_2.5.1             stringi_1.8.4       
 [31] parallelly_1.41.0    rpart_4.1.23         lubridate_1.9.4     
 [34] Rcpp_1.0.14          iterators_1.0.14     knitr_1.49          
 [37] future.apply_1.11.3  zoo_1.8-12           base64enc_0.1-3     
 [40] Matrix_1.7-0         splines_4.4.0        nnet_7.3-19         
 [43] timechange_0.3.0     tidyselect_1.2.1     yaml_2.3.10         
 [46] rstudioapi_0.17.1    timeDate_4041.110    doParallel_1.0.17   
 [49] codetools_0.2-20     listenv_0.9.1        lattice_0.22-6      
 [52] tibble_3.2.1         plyr_1.8.9           GLDEX_2.0.0.9.3     
 [55] withr_3.0.2          evaluate_1.0.3       foreign_0.8-86      
 [58] future_1.34.0        survival_3.5-8       RcppParallel_5.1.8  
 [61] fitdistrplus_1.1-8   pillar_1.10.1        rsconnect_0.8.26    
 [64] checkmate_2.3.2      foreach_1.5.2        stats4_4.4.0        
 [67] generics_0.1.3       munsell_0.5.1        scales_1.3.0        
 [70] globals_0.16.3       class_7.3-22         CalibratR_0.1.2     
 [73] glue_1.8.0           tools_4.4.0          data.table_1.16.4   
 [76] SparseM_1.84         ModelMetrics_1.2.2.2 gower_1.0.2         
 [79] forcats_1.0.0        mvtnorm_1.2-5        grid_4.4.0          
 [82] rbibutils_2.3        wesanderson_0.3.7    ipred_0.9-15        
 [85] colorspace_2.1-1     patchwork_1.2.0      nlme_3.1-164        
 [88] RNGforGPD_1.1.0      htmlTable_2.4.3      spacefillr_0.3.3    
 [91] Formula_1.2-5        cli_3.6.3            gp_1.1              
 [94] lava_1.8.1           corpcor_1.6.10       ggcorrplot_0.1.4.1  
 [97] gtable_0.3.6         digest_0.6.37        caret_7.0-1         
[100] TH.data_1.1-2        htmlwidgets_1.6.4    htmltools_0.5.8.1   
[103] lifecycle_1.0.4      hardhat_1.4.0        MASS_7.3-60.2
```
...
Van Calster, B., Collins, G. S., Vickers, A. J., Wynants, L., Kerr, K. F., Barreñada, L., ... & Steyerberg, E. W. (2024). Performance evaluation of predictive AI models to support medical decisions: Overview and guidance. [arXiv:2412.10288](https://doi.org/10.48550/arXiv.2412.10288)
...
Collins, G. S., Dhiman, P., Ma, J., Schlussel, M. M., Archer, L., Van Calster, B., ... & Riley, R. D. (2024). Evaluation of clinical prediction models (part 1): from development to external validation. *bmj, 384*. [DOI](http://dx.doi.org/10.1136/bmj‑2023‑074819)
